...
async function update(req, res) {
  const url =
    'https://igsyv1z1xi-2.algolianet.com/1/indexes/*/queries?' +
      ...
      requests: [
        {
          ...
          params: Object.keys(params)
            .map((key) => `${key}=${urlencode(JSON.stringify(params[key]))}`)
            .join('&'),
        },
      ],
    };
    ...
    for (const course of courses) {
      if (courseIndex.has(course.objectID)) {
        ...
        courseList.push({
          ...
          url: course.marketing_url.replace('https://www.edx.org/course/', ''),
          image: course.card_image_url.replace(
